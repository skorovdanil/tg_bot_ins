...
async def main_worker_menu(callback: types.CallbackQuery, state: FSMContext):
...
@router.callback_query(F.data == "worker_menu_out_photo")
async def shulder_worker_menu(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.delete()
    await callback.message.answer(f"Выберите, что вам нужно", reply_markup=nav.WorkerMainMenu)
    await state.set_state(Gen.worker_action)
...
@router.callback_query(F.data == "update_shulder_worker_menu")
async def shulder_worker_menu(callback: types.CallbackQuery, state: FSMContext):
...
async def update_region(callback: types.CallbackQuery, state: FSMContext):
    regions = await nav.db_region()
    await callback.message.delete()
    await callback.message.answer_photo(
        photo="https://sun9-14.userapi.com/impg/IssyepRA_sRxWEgxWWqCgKjie5r9s_f-hOlbcw/G4TSsYRhlcs.jpg?size=1280x1152&quality=95&sign=f4ef9b29c9c850528f0fafb17b790c58&type=album",
